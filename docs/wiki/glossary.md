...
| Pending flare | A flare body the CLI saved to `~/.local/state/graywolf/pending-flare-<unix-ts>.json` (mode 0600) after the server returned 5xx. The operator retries by re-running `graywolf flare`. | [`../../graywolf/pkg/diagcollect/submit/store.go`](../../graywolf/pkg/diagcollect/submit/store.go) |
